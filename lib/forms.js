...
var Form = Class.extend({
    ...
    createFields: function createFields(fields) {
        function getField(field, name) {
            ...
            field.label = !field.label && field.label !== false ? camelCaseToRegularForm(name) : field.label;
    ...
    html: function html(values, options) {
        ...
        console.log('ret', ret)
        return htmlTag('form', this.formAttributes(values), ret);
    },
    ...
    errorHtml: function errorHtml(values, options) {
        ...
        if (options.errorMessage && !options.renderSuccess) {
            return htmlTag('div', { class: 'formError' }, htmlTag('p', {}, String(options.errorMessage)));
        }
        var validation = this.validate(values);
        if (validation instanceof Error && !options.renderSuccess) {
            return htmlTag('div', { class: 'formError' }, htmlTag('p', {}, validation.message));
        } else if (options.renderSuccess) {
            return htmlTag('div', { class: 'formSuccess' }, htmlTag('p', {}, this.options.successMessage));
        } else {
            return '';
        }
    },
    ...
    buttonsHtml: function buttonsHtml() {
        var options = this.options,
            button = htmlTag(options.submitTagName, options.submitAttrs, options.submitLabel);

        if (options.buttonSetTagName === ('ul' || 'ol')) {
            button = htmlTag('li', false, button);
        }
    ...
    formAttributes: function formAttributes() {
        ...
        if (this.options.classes && this.options.classes.length) {
            attrs.class = mungeClassValues(this.options.classes);
        }
    ...
    fieldHtml: function fieldHtml(field, value, options) {
        return htmlTag(
            ...
            _.extend({
                'class': mungeClassValues(field.classes, 'field'),
...
var Field = Class.extend({
    ...
    errorHtml: function errorHTML(value, options) {
        ...
        return error instanceof Error ? htmlTag('div', { class: 'fieldError' }, error.message) : '';
    },
    ...
    labelHtml: function labelHtml(value, options) {
        ...
        if (!this.required && !this.noOptionalIndicator) {
            label += htmlTag('span', { 'class': 'optionalIndicator' }, '(optional)');
        }
        return htmlTag('label', { for: forVal }, label);
    },

    widgetHtml: function widgetHtml(value) {
        return htmlTag(this.tagName, this.widgetAttributes(value));
    },
        ...
        widgetHtml: function selectWidgetHtml(value) {
            ...
            _.each(this.choices, function (name, index) {
                if (_.isObject(name)) {
                    ...
                    html += htmlTag('option', { value: origName[name] }, name);
                }
                else {
                    html += htmlTag('option', { value: name }, name);
                }
            }, this);
            return htmlTag(this.tagName, this.widgetAttributes(value), html);
        },
