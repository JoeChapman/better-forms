...
var Form = Class.extend({
    ...
    constructor: function Form(name, fields, options) {
        ...
        this.options = _.defaults(options || {}, this.options, {
            ...
            predirect: this.predirect.bind(this),
    ...
    createField: function createField(field, name) {
        ...
        return new formFields[field.type](field, this.options);
    },
    ...
    predirect: function predirect(req, res, callback) { callback(null); },
        ...
        if (req.xhr) {
            this.options.getValues(req, res, function (err, formValues) {
                if (this.options.redirectUrl) {
                    this.options.predirect(req, res, function (redirectUrl) {
                        res.status(200).json({ redirect: redirectUrl || this.options.redirectUrl });
                    }.bind(this));
                } else {
            ...
            this.options.predirect(req, res, function (redirectUrl) {
                redirectUrl = redirectUrl || this.options.redirectUrl;
                if (req && req.session && !redirectUrl) {
                    (req.session.forms || (req.session.forms = {}))[this.formName] = formObject;
                } else if (redirectUrl && req.session.forms) {
                    delete req.session.forms[this.formName];
                }
                res.redirect(redirectUrl || req.path);
            }.bind(this));
        }
    },
    ...
    formAttributes: function formAttributes() {
        var attrs = {
            ...
            acceptable = ['id', 'classes', 'role', 'novalidate', 'autocomplete', 'enctype', 'accept-charset'];

        _.each(this.options, function (val, key) {
            if (acceptable.indexOf(key) !== -1 && this.options[key]) {
                if (key === 'classes') {
